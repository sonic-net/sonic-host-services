...
import switch_platform_init as ppi


class TestSwitchPlatformInit(unittest.TestCase):

  def test_init(self):
    with mock.patch('redis.Redis', autospec=True) as mock_redis:
      ppi.SwitchPlatformInit()
  ...
  @mock.patch(
      'switch_platform_init.SwitchPlatformInit.__init__',
  ...
  def test_primary_version_dict(self):
    platform = ppi.SwitchPlatformInit()
    platform.primary_version_dict = {
        'build_version': 'SONiC-OS-202311.0',
  ...
  def test_available_network_stacks_with_same_current_and_next(self):
    ...
    Current: SONiC-OS-202311.0-44c7e0b3
    Next: SONiC-OS-202311.0-44c7e0b3
    Available:
    SONiC-OS-202311.0-44c7e0b3
    SONiC-OS-202311.1-44c7e0b3
    """
  ...
  def test_write_chassis_info_paths(self):
    platform_init = ppi.SwitchPlatformInit()
    platform_init.primary_version_dict = {
        'build_version': 'SONiC-OS-202311.0',
    }
    platform_init.state_db = mock.MagicMock()
    platform_init.write_chassis_info_paths()
    platform_init.state_db.hset.assert_called_once_with(
        ppi.CHASSIS_INFO,
        ppi.FW_VERSION,
        'SONiC-OS-202311.0',
    )
  ...
  def test_populate_version_dictionary(self):
    ...
    with mock.patch.object(
      ...
      dictionary = ppi.SwitchPlatformInit().populate_version_dictionary(file_name)
  ...
  def test_populate_version_dictionary_with_separator(self):
    ...
    with mock.patch.object(
      ...
      dictionary = ppi.SwitchPlatformInit().populate_version_dictionary(
          file_name, '='
      )
      self.assertEqual(dictionary, expected_dictionary)

  @mock.patch(
      'switch_platform_init.SwitchPlatformInit.populate_version_dictionary',
      side_effect=ValueError,
  )
  def test_populate_version_dictionary_assert_value_error(self, mock_error):
    file_name = os.path.join(test_path, 'test_data', 'test_file.txt')
    with self.assertRaises(ValueError):
      ppi.SwitchPlatformInit().populate_version_dictionary(file_name)
  ...
  def test_populate_version_dictionaries(self):
    ...
    platform.run_command = mock.MagicMock()
    with mock.patch(
        'switch_platform_init.os.path.exists', return_value=True, autospec=True
    ):
      with mock.patch(
          'switch_platform_init.os.makedirs', autospec=True
      ) as mock_makedirs:
        with mock.patch(
            'switch_platform_init.shutil.rmtree', autospec=True
        ) as mock_rmtree:
  ...
  def test_write_sw_network_stack_paths(self):
    platform = ppi.SwitchPlatformInit()
    platform.primary_version_dict = {
        'build_version': 'switch_primary_version',
    }
    platform.alternate_versions_dict = {
        'image1': {'build_version': 'switch_alternate_version'}
    }
    platform.state_db = mock.MagicMock()
    ...
    platform.state_db.hset.assert_any_call(
        ppi.SW_COMP_INFO_STACK0, ppi.SOFTWARE_VERSION, 'switch_primary_version'
    )
    platform.state_db.hset.assert_any_call(
  ...
  def test_write_sw_os_paths(self):
    ...
    platform.alternate_versions_dict = {
        'image1': {
            'build_version': 'switch_alternate_version',
  ...
  @mock.patch(
      'switch_platform_init.SwitchPlatformInit.write_system_paths',
      side_effect=OSError,
  )
  def test_write_system_path_assert_os_error(self, mock_error):
    with self.assertRaises(OSError):
      ppi.SwitchPlatformInit().write_system_paths()
    mock_error.assert_called_once()

  def test_get_network_name(self):
    platform = ppi.SwitchPlatformInit()
    platform.run_command = mock.MagicMock()
    expected_network_name = 'switch1.example.com'
    platform.run_command.return_value = 'switch1.example.com'
  ...
  def test_write_host_paths(self):
    ...
    platform.get_network_name.return_value = 'switch1.example.com'
    platform.config_db.hget.return_value = 'switch1.example.com'
    platform.write_host_paths()
    self.assertEqual(platform.network_name, 'switch1.example.com')
    platform.state_db.hset.assert_has_calls([
  ...
  def test_write_switchinit_done(self):
    platform = ppi.SwitchPlatformInit()
    platform.state_db = mock.MagicMock()
    platform.write_switchinit_done()
    platform.state_db.hset.assert_called_once_with(
        ppi.SWITCH_PLATFORM_INIT_TBL,
        ppi.SWITCH_PLATFORM_READY,
        ppi.SWITCH_PLATFORM_READY,
    )
