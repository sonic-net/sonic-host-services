...
class TestDebugInfo:
    ...
    def test_check_ready(self):
        ...
        rc, msg = self.debug_info_module.check("artifact")
        assert rc == 0
        assert "artifact ready" in msg.lower()
    ...
    def test_collect_artifacts_invalid_json(self):
        req = '{"component": "orchagent", "log_level": "alert"' # malformed JSON}
        with mock.patch("debug_info.swsscommon.DBConnector"), \
                mock.patch("debug_info.swsscommon.NotificationProducer"), \
                mock.patch("debug_info.swsscommon.NotificationConsumer"), \
                mock.patch("debug_info.swsscommon.Select"):
            rc, out = DebugInfo.collect_artifacts(req, "1234", "boardX", "hostA")
        assert rc != 0
    ...
    def test_collect_artifacts_happy_path(self):
        ...
        with mock.patch("debug_info.swsscommon.DBConnector"), \
                mock.patch("debug_info.swsscommon.NotificationProducer"), \
                mock.patch("debug_info.swsscommon.NotificationConsumer"), \
                mock.patch("debug_info.swsscommon.Select") as mock_select, \
                mock.patch("debug_info.os.makedirs"), \
                mock.patch("debug_info.shutil.rmtree"), \
                mock.patch.object(DebugInfo, "_send_component_request", return_value={"compA": "ok"}), \
                mock.patch.object(DebugInfo, "_collect_counter_artifacts"), \
                mock.patch.object(DebugInfo, "_collect_teamdctl_data"), \
                mock.patch.object(DebugInfo, "_get_component_response"), \
                mock.patch.object(DebugInfo, "_save_persistent_storage"), \
                mock.patch.object(DebugInfo, "_run_command", return_value=(0, "out", "")):

                # stub addSelectable so it doesn’t error
                mock_select.return_value.addSelectable.return_value = None

                rc, msg = DebugInfo.collect_artifacts(
                        req, "1234", "boardX", "hostA"
                        )
        assert rc == 0
        assert msg.endswith(".tar.gz")
        assert "hostA_1234" in msg



    def test_collect_artifacts_thread_success(self):
        with mock.patch("debug_info.DebugInfo.collect_artifacts", return_value=(0, "ok")) as m:
            dbg = DebugInfo.__new__(DebugInfo) #bypass __init__
            dbg._ongoing_thread = "thread1"
            dbg._lock = threading.Lock()

            DebugInfo._collect_artifacts_thread(dbg, "{}", "1234", "boardX", "hostA")

            m.assert_called_once_with("{}", "1234", "boardX", "hostA")
            assert dbg._ongoing_thread == ""
