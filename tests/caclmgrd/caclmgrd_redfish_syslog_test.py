...
class TestCaclmgrdRedfishSyslog(TestCase):
    ...
    def test_handle_redfish_feature_events(self, fs):
        """Drive the FEATURE-table handler: enable transition, disable transition,
        unrelated key ignored, and same-state no-op. The exception is only correct
        across a runtime feature toggle if this flag tracks the state."""
        if not os.path.exists(DBCONFIG_PATH):
            fs.create_file(DBCONFIG_PATH)
        ...
        notif = set()
        daemon.handle_redfish_feature_events(
        ...
        # unrelated FEATURE event: redfish flag untouched, nothing queued
        notif = set()
        daemon.handle_redfish_feature_events(
