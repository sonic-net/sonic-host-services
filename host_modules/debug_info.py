...
class DebugInfo(host_service.HostModule):
  ...
  def _send_component_request(component, dir_name, log_level, producer):
    ...
    logger.debug(f"components dict: {components}")
    return components
  ...
  def _save_persistent_storage(artifact_name: str) -> None:
    logger.debug("Inside save_persistent_storage()")
  ...
  def collect_artifacts(req: str, timestamp: str, board_type: str,
    ...
    logger.debug("collect_artifacts() args:%s %s %s\n",req, board_type, hostname)
  ...
  def collect(self, options):
    ...
    container_artifact_name = ARTIFACT_DIR_CONTAINER + "/" + artifact_name
    with self._lock:
  ...
  def ack(self, options):
    ...
    # host.
    artifact = ARTIFACT_DIR + options[0].removeprefix(ARTIFACT_DIR_CONTAINER)
    try:
      os.remove(artifact)
