...
class DebugInfo(host_service.HostModule):
  ...
  def collect(self, options):
    ...
    try:
      print("Raw input to collect():", options)
      print("DEBUG options =", options)
      print("DEBUG options[0] =", options[0] if options else "None")
      json.loads(options)
    except json.JSONDecodeError:
    ...
    try:
      t = threading.Thread(
          ...
          args=(options[0], timestamp, self._board_type, self._hostname))
      t.start()
  ...
  def check(self, options):
    with self._lock:
      if self._ongoing_thread == options[0]:
