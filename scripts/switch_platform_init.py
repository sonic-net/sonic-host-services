...
SWITCH_PLATFORM_INIT_TBL = "SWITCH_PLATFORM_INIT"
SWITCH_PLATFORM_READY = "ready"
...
class SwitchPlatformInit:
  ...
  def populate_version_dictionaries(self):
    ...
    for image in available_images:
      if image not in current_image:
        ...
        # TODO: Remove this when we fix sonic-installer for systemd
  ...
  def get_network_name(self) -> str:
    ...
    Returns:
      The fully-qualified name of the switch.
    """
    try:
  ...
  def write_switchinit_done(self) -> None:
    """Write SWITCH_INIT_DONE to STATE_DB."""
    # The last thing to do, indicating we are done initializing.
    self.state_db.hset(
        SWITCH_PLATFORM_INIT_TBL, SWITCH_PLATFORM_READY, SWITCH_PLATFORM_READY
    )


def main() -> None:
  """Write SWITCH platform specific init info to STATE/APPL_STATE_DB."""
  platform = SwitchPlatformInit()
