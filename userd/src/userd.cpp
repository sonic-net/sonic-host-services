...
class SystemCommand {
public:
    static bool execute(const std::vector<std::string>& cmd) {
        if (cmd.empty()) {
            return false;
        }
...
public:
        ...
        for (const auto& entry : user_data) {
            ...
            for (const auto& field : fvs) {
                ...
                if (field_name == "role") {
                    ...
                    // Parse SSH keys (stored as comma-separated values)
                    std::stringstream ss(field_value);
                    std::string key;
                    while (getline(ss, key, ',')) {
                        if (!key.empty()) {
                            user.ssh_keys.push_back(key);
                        }
                    }
                }
            }
    ...
    bool set_user_groups(const std::string& username, const std::string& role) {
        ...
        SWSS_LOG_DEBUG("Added user %s to managed group and role %s groups", username.c_str(), role.c_str());
        return true;
    }
    ...
    void perform_consistency_check() {
        ...
        try {
            ...
            SWSS_LOG_ERROR("Consistency check failed: %s", e.what());
        }
    }
    ...
    bool update_user(const std::string& username, const UserInfo& user_config) {
        try {
            ...
            SWSS_LOG_ERROR("Failed to update user %s: %s", username.c_str(), e.what());
            return false;
        }
    }
    ...
    void handle_config_change(const std::string& table, const std::string& key, const std::map<std::string, std::string>& data) {
        try {
                ...
                if (!data.empty()) {
                    ...
                    for (const auto& field : data) {
                        if (field.first == "role") {
                            ...
                            std::string ssh_key;
                            while (getline(ss, ssh_key, ',')) {
                                if (!ssh_key.empty()) {
                                    user.ssh_keys.push_back(ssh_key);
                                }
                            }
                        }
                    }
            ...
            SWSS_LOG_ERROR("Failed to handle config change: %s", e.what());
        }
    }
};
